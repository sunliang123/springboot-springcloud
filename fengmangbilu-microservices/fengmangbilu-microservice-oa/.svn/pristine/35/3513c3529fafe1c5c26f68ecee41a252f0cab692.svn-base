...
public class DefaultCreditClient implements ICreditClient {
	...
	@Override
	public ICreditClient getPerson() throws ReportException {
	...
	@Override
	public ICreditClient getOsta() throws ReportException {
		Conditions conditions = profile.getConditions();
		Condition condition = conditions.getCondition();
		List<Item> items = condition.getItem();
	...
	@Override
	public ICreditClient getRisk() throws ReportException {
		Conditions conditions = profile.getConditions();
		Condition condition = conditions.getCondition();
		List<Item> items = condition.getItem();
	...
	@Override
	public ICreditClient getRiskAssess() throws ReportException {
		Conditions conditions = profile.getConditions();
		Condition condition = conditions.getCondition();
		List<Item> items = condition.getItem();
	...
	private CisReports doAction(Conditions conditions) throws ReportException {
		Call call = null;
		try {
			call = (Call) new Service().createCall();
		} catch (ServiceException e) {
			logger.error(e.getMessage());
			throw ReportException.SERVICE;
		}
		...
		try {
			...
			logger.error(e.getMessage());
			throw ReportException.CONDITION;
		}
		String resultXml;
		try {
		...
		try {
			...
			logger.error(e.getMessage());
			throw ReportException.RESULT;
		}
		if (!StringUtils.equals(result.getStatus(), "1")) {
			throw new ReportException(result.getErrorMessage());
		}
		...
		try {
			...
			logger.error(e.getMessage());
			throw ReportException.UNZIP;
		}
		return cisReports;
	}

	private void policeCheck(CisReports cisReports) throws ReportException {
		...
		if (StringUtils.equals(policeInfo.getTreatResult(), "2")) {
			throw ReportException.IDCARD_DIFFER;
		}
		if (!StringUtils.equalsAny(policeInfo.getTreatResult(), "1", "2")) {
			throw ReportException.IDCARD_UNFOUND;
		}
	}
}
